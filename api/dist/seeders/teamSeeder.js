...
]; // as const → tuple, mỗi phần tử là literal type, không bao giờ undefined khi index trong bounds
const POSITIONS = [
    "goalkeeper",
    "defender",
    "defender",
...
export async function seedTeams(db, adminUserId, seasonId) {
    ...
    for (let t = 0; t < TEAMS_DATA.length; t++) {
        ...
        for (let p = 0; p < POSITIONS.length; p++) {
            ...
            const playerName = PLAYER_NAMES[nameIdx % PLAYER_NAMES.length];
            nameIdx++;
        ...
        console.log(`  → Team "${team.name}" #${team.id} | SeasonTeam #${seasonTeam.id} | ${playerIds.length} players`);
    }
    return results;
}
