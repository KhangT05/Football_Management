 ...
 */
export async function seedTournamentRule(db, tournamentId, seasonId) {
    ...
    if (season.tournament_id !== tournamentId) {
        throw new Error(`seedTournamentRule: season #${seasonId} thuộc tournament #${season.tournament_id}, ` +
            `không phải tournament #${tournamentId} truyền vào — kiểm tra lại thứ tự gọi seeder.`);
    }
    if (season.tournament_rule_id != null) {
        console.log(`[TournamentRuleSeeder] Season #${seasonId} đã có tournament_rule_id=${season.tournament_rule_id} ` +
            `(gán từ seedSeasonConfigurable) — bỏ qua, KHÔNG tạo rule generic hay ghi đè.`);
        return { ruleId: season.tournament_rule_id };
    }
    throw new Error(`seedTournamentRule: season #${seasonId} không có tournament_rule_id. ` +
...
//# sourceMappingURL=tournamnetRuleSeeder.js.map