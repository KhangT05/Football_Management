...
async function main() {
    ...
    const { groupStagePhaseId, groupIdByLetter } = await seedGroupStage(prisma, seasonId, teamIdByName, seasonTeamIdByTeamId);
    // 9. Match vòng bảng + TeamStanding
    const { topTwoByGroup, createdMatches: groupMatches } = await seedGroupMatchesAndStandings(prisma, groupStagePhaseId, groupIdByLetter, teamIdByName, seasonId, venueIds);
    // 10. Knockout
    const { championTeamId, allMatches: knockoutMatches } = await seedKnockoutBracket(prisma, seasonId, topTwoByGroup, venueIds);
