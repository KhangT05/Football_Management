...
export async function applySuspensionsAndFines(db, seasonId, rule) {
    const stats = await db.playerStatistic.findMany({ where: { season_id: seasonId } });
    let suspendedCount = 0;
    let finedCount = 0;
    for (const s of stats) {
        const shouldSuspend = s.yellow_cards_since_reset >= rule.yellowCardsSuspension || s.red_cards > 0;
        const fine = s.yellow_cards * rule.finePerYellowCard + s.red_cards * rule.finePerRedCard;
        const data = {};
        if (shouldSuspend) {
    ...
    console.log(`[SuspensionSeeder] Season #${seasonId}: ${suspendedCount} cầu thủ bị treo giò, ${finedCount} cầu thủ có tiền phạt.`);
}
//# sourceMappingURL=suspensionSeeder.js.map