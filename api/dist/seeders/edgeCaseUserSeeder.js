...
export async function seedOrphanLeaders(db, roleMap, count) {
...
export async function seedFreeAgentPlayers(db, roleMap, count) {
    const passwordHash = await bcrypt.hash(DEFAULT_PASSWORD, BCRYPT_ROUNDS);
    const playerIds = [];
    for (let i = 1; i <= count; i++) {
        ...
        const player = await db.player.upsert({
            ...
            create: {
                ...
                position: [PlayerPosition.goalkeeper, PlayerPosition.defender, PlayerPosition.midfielder, PlayerPosition.forward][randInt(0, 3)],
                nationality: "Vietnam",
            },
        });
 ...
 */
export async function seedUnregisteredTeam(db, adminUserId, teamName, classIdByName) {
 ...
 */
export async function seedUnderStaffedSeasonTeam(db, seasonTeamId, teamNameForEmail, squadSize) {
 ...
 */
export async function seedIncompleteApprovalStates(db, seasonTeamId) {
    ...
    console.log(`[EdgeCaseUserSeeder] season_team #${seasonTeamId}: ${pendingCount} pending, ${rejectedCount} rejected, ` +
 ...
 */
export async function seedRoleStackedUser(db, roleMap, label) {
    const passwordHash = await bcrypt.hash(DEFAULT_PASSWORD, BCRYPT_ROUNDS);
 ...
 */
export async function seedInactiveUserWithLiveRoster(db, seasonTeamId, label, jerseyNumber) {
    const passwordHash = await bcrypt.hash(DEFAULT_PASSWORD, BCRYPT_ROUNDS);
 ...
 */
export async function seedUnverifiedEmailPendingPlayer(db, seasonTeamId, label, jerseyNumber) {
    const passwordHash = await bcrypt.hash(DEFAULT_PASSWORD, BCRYPT_ROUNDS);
...
//# sourceMappingURL=edgeCaseUserSeeder.js.map