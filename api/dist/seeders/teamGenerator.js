...
export function generateGroupLetters(groupCount) {
    if (groupCount <= 0)
        throw new Error(`generateGroupLetters: groupCount phải > 0, nhận ${groupCount}`);
 ...
 */
export function generateTeamNamesFromOffset(count, offset) {
    const pool = [...CITY_POOL, ...CITY_POOL.map((c) => `${c} B`)]; // nhân đôi pool cho subset lệch offset lớn
    if (offset + count > pool.length) {
        throw new Error(`generateTeamNamesFromOffset: offset=${offset} + count=${count} vượt quá pool (${pool.length}) — thêm city vào CITY_POOL`);
    }
    return pool.slice(offset, offset + count).map((city) => `CLB ${city}`);
}
export function generateGroups(teamNames, groupCount = GROUP_COUNT) {
...
export function generateCrossPairingForTwoGroups(letters) {
    const [w, x] = letters;
    return [[w, x], [x, w]];
}
const VENUE_POOL = [
