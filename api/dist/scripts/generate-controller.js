...
function parseRelations(raw) {
    return raw.split(",").map((part) => {
        ...
        const [field, entity, type, modifier] = segments;
        if (type !== "n-n") {
...
if (!entityNameArg) {
    console.error("Usage: tsx scripts/generate-controller.ts <EntityName> " +
...
const relations = relationsArg ? parseRelations(relationsArg) : [];
 ...
 */
function implicitRelationMethods(rel) {
 ...
 */
function explicitRelationMethods(rel) {
    const field = rel.field;
    const related = rel.entity;
    const relPath = pluralize(field);
    const capField = capitalize(field);
...
function controllerTemplate() {
    ...
    if (hasAnyRelation && !tsoaImports.includes("Delete"))
        tsoaImports.push("Delete");
    if (hasExplicitRelations && !tsoaImports.includes("SuccessResponse"))
        tsoaImports.push("SuccessResponse");
    if (useRelationService && hasImplicitRelations && !tsoaImports.includes("Put"))
        tsoaImports.push("Put");
    // ── Imports ──
    ...
    if (extraSafeTypes.length > 0) {
        ...
        for (const t of extraSafeTypes)
            lines.push(`// import { type ${t} } from "...";`);
    }
    if (!readonly) {
        const dtoNames = [`type Create${entityName}Dto`, `type Update${entityName}Dto`];
        for (const rel of relations) {
            if (rel.explicit)
                dtoNames.push(`type ${entityName}${rel.entity}Dto`);
            if (!rel.explicit && useRelationService)
                dtoNames.push(`type ${capitalize(rel.entity)}IdsDto`);
        }
...
console.log(`  Flags     : readonly=${readonly}, noDelete=${noDelete}, auth=${auth}, authUser=${authUser}, ` +
