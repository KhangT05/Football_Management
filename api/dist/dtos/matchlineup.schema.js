...
}).refine((d) => d.minute_out === undefined || d.minute_in !== undefined, { message: 'minute_out requires minute_in', path: ['minute_out'] });
export const registerLineupSchema = z.object({
    match_id: z.number().int().positive(),
    team_id: z.number().int().positive(),
    players: z.array(lineupEntrySchema).min(1).max(30),
}).refine((d) => d.players.filter(p => p.is_captain).length <= 1, { message: 'Chỉ được 1 captain', path: ['players'] }).refine((d) => new Set(d.players.map(p => p.player_id)).size === d.players.length, { message: 'player_id trùng trong lineup', path: ['players'] });
export const updateLineupEntrySchema = z.object({
    match_id: z.number().int().positive(),
    team_id: z.number().int().positive(),
    player_id: z.number().int().positive(),
}).merge(lineupEntrySchema);
//# sourceMappingURL=matchlineup.schema.js.map