...
export class PaymentService {
    ...
    async initiateManualPayment(leaderId, seasonTeamId) {
        const seasonTeam = await this.prisma.seasonTeam.findUnique({
        ...
        if (!seasonTeam.season.is_registration_open)
            throw createAppError('CONFLICT', 'Mùa giải đã đóng đăng ký');
        const hasBankInfo = !!(seasonTeam.season.bank_id &&
        ...
        const isPendingRecent = latestPayment?.status === PaymentStatus.pending &&
            Date.now() - latestPayment.created_at.getTime() <=
                PaymentService.PAYMENT_EXPIRE_MINUTES * 60_000;
        if (isPendingRecent) {
            return {
    ...
    async queryTransaction(paymentId) {
        ...
        if (!payment.transaction_ref)
            throw createAppError('CONFLICT', 'Payment này tạo qua chuyển khoản thủ công, không có giao dịch VNPay để đối soát');
        const now = new Date();
    ...
    async refundPayment(paymentId, adminId, input) {
        ...
        if (!payment)
            throw createAppError('NOT_FOUND', `Payment ${paymentId} không tồn tại`);
        if (!payment.transaction_ref)
            throw createAppError('CONFLICT', 'Payment này xác nhận qua chuyển khoản thủ công, không thể refund tự động qua VNPay');
        if (payment.status !== PaymentStatus.confirmed)
        ...
        if (input.amount > Number(payment.amount))
            throw createAppError('CONFLICT', `Số tiền refund (${input.amount}) vượt quá số tiền đã thanh toán (${Number(payment.amount)})`);
        ...
        if (input.type === 'full' && input.amount !== Number(payment.amount))
            throw createAppError('CONFLICT', 'Hoàn tiền toàn phần (full) phải bằng đúng số tiền đã thanh toán');
