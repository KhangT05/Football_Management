...
function renderScoreBlock(doc, report) {
    ...
    doc.fontSize(18).font('Helvetica-Bold').text(`${report.home.name}   ${s.homeFinal} - ${s.awayFinal}   ${report.away.name}`, { align: 'center' });
    doc.moveDown(0.3);
    const parts = [`HT: ${s.homeHalfTime ?? '-'} - ${s.awayHalfTime ?? '-'}`];
    if (s.homeExtraTime != null)
        parts.push(`ET: ${s.homeExtraTime}-${s.awayExtraTime}`);
    if (s.homePenalty != null)
        parts.push(`Pen: ${s.homePenalty}-${s.awayPenalty}`);
...
function renderFooter(doc, report) {
    doc.moveDown(1.5);
    doc.fontSize(8).font('Helvetica-Oblique').fillColor('#888888').text(`Biên bản được xuất tự động lúc ${new Date().toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })}`, { align: 'right' });
}
