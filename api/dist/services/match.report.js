...
import { buildGoalsTimeline, buildMatchReportPlayerRows, isCreditedToHomeTeam, } from '../helper/match.helper.js';
...
export class MatchReportService {
    ...
    async buildMatchReport(matchId) {
        ...
        const playerNameLookup = new Map(match.matchLineups.map(l => [l.player_id, l.player.user.name]));
        const goalsTimeline = buildGoalsTimeline(match.events, match.home_team_id, match.away_team_id, playerNameLookup);
        ...
        return {
            ...
            lineups: {
                home: buildMatchReportPlayerRows(match.matchLineups, jerseyLookup, match.events, match.home_team_id),
                away: buildMatchReportPlayerRows(match.matchLineups, jerseyLookup, match.events, match.away_team_id),
            },
            goalsTimeline,
        };
    }
    ...
    computeHalfTimeScore(match) {
        let home = 0;
        let away = 0;
        for (const e of match.events) {
            if (e.period !== MatchPeriod.first_half)
                continue;
            if (e.team_id == null)
                continue;
...
//# sourceMappingURL=match.report.js.map