...
export class ScheduleService extends ScheduleEngine {
    ...
    async findUnscheduledMatchesInRound(seasonId, groupId, round) {
    ...
    async getAvailableSlotsForMatch(seasonId, matchId, options) {
        if (options.venueIds.length === 0)
            throw createAppError('VALIDATION_ERROR', 'venueIds không được rỗng');
        if (!options.dailyStartTime || !options.dailyEndTime)
            throw createAppError('VALIDATION_ERROR', 'dailyStartTime/dailyEndTime không được rỗng');
        ...
        const bufferMinutes = options.bufferMinutes ?? DEFAULT_VENUE_BUFFER_MINUTES;
        const takenWindows = await this.loadTakenVenueWindows(options.venueIds, season.start_date, season.end_date);
        if (match.scheduled_at && match.venue_id) {
            ...
            if (windows) {
                takenWindows.set(match.venue_id, windows.filter(w => w.start !== ownWindowStart));
            }
        }
        const pool = this.buildSlotPool(options.venueIds, season.start_date, season.end_date, options.dailyStartTime, options.dailyEndTime, bufferMinutes, takenWindows, options.excludedDates ?? []);
        ...
        const playedTimesByTeam = new Map();
        for (const m of scheduledMatches) {
            const t = m.scheduled_at.getTime();
            for (const tid of [m.home_team_id, m.away_team_id]) {
                if (!playedTimesByTeam.has(tid))
                    playedTimesByTeam.set(tid, []);
                playedTimesByTeam.get(tid).push(t);
            }
        }
        const { conflictMap, occupiedWindows } = await this.loadPlayerConflictContext(this.prisma, [match.home_team_id, match.away_team_id], [matchId]);
        const candidates = this.findAvailableSlotsForMatch(pool, match.home_team_id, match.away_team_id, playedTimesByTeam, phase.min_rest_days_per_team ?? 3, conflictMap, occupiedWindows, options.limit ?? 50);
