import { GroupStatus, LineupType, MatchEventType, MatchPeriod, MatchResultType, MatchStatus, PhaseFormat, PhaseStatus, PhaseType, SeasonTeamStatus, } from '../generated/prisma/client.js';
...
export class WorkflowService {
    ...
    clockBase = new Date();
    matchSlot = 0;
    matchIntervalMinutes = DEFAULT_MATCH_INTERVAL_MINUTES;
    constructor(prisma, knockoutService, matchLifecycleService, matchResultService, standingsService) {
        this.prisma = prisma;
    ...
    async run(config) {
        ...
        // ── 5. Group Phase A ───────────────────────────────────────────────
        const { phaseId: groupAPhaseId, groupId: groupAId } = await this._createGroupPhase(seasonId, 'Group Stage A', groupATeams);
        this._log(`Group A phase=${groupAPhaseId} group=${groupAId}`);
        const groupAMatches = await this._createRoundRobinMatches(groupAPhaseId, groupAId, groupATeams, config.venueId);
        ...
        // ── 6. Group Phase B ───────────────────────────────────────────────
        const { phaseId: groupBPhaseId, groupId: groupBId } = await this._createGroupPhase(seasonId, 'Group Stage B', groupBTeams);
        this._log(`Group B phase=${groupBPhaseId} group=${groupBId}`);
        const groupBMatches = await this._createRoundRobinMatches(groupBPhaseId, groupBId, groupBTeams, config.venueId);
        ...
        const { phaseId: knockoutPhaseId, match: finalMatch } = await this._createAndRunFinal(seasonId, groupAWinner.teamId, groupBWinner.teamId, config.venueId, finalDelaySlots);
    ...
    async _registerSeasonTeams(seasonId, teamIds) {
    ...
    // ─── GROUP PHASE ──────────────────────────────────────────────────────────
    async _createGroupPhase(seasonId, name, teams) {
    ...
    async _createRoundRobinMatches(phaseId, groupId, teams, venueId) {
    ...
    // ─── MATCH EXECUTION ──────────────────────────────────────────────────────
    async _executeMatches(matches, teamMap, venueId) {
        const reports = [];
    ...
    async _playMatch(m, teamMap, venueId) {
        ...
        await this.matchLifecycleService.finalizeMatch(m.id, { resultType: MatchResultType.full_time }, NO_SCHEDULE);
     ...
     */
    async _createLineup(matchId, homeTeamId, awayTeamId) {
        await Promise.all([
            this._createTeamLineup(matchId, homeTeamId),
            this._createTeamLineup(matchId, awayTeamId),
        ]);
    }
    async _createTeamLineup(matchId, teamId) {
        ...
        if (players.length === 0)
            return; // team chưa có player → skip silently
     ...
     */
    async _getMatchPlayers(matchId, teamId) {
    ...
    // ─── EVENT RECORDING ──────────────────────────────────────────────────────
    async _recordGoals(matchId, teamId, count, playerIds, period) {
    ...
    // ─── STANDINGS ────────────────────────────────────────────────────────────
    async _getStandings(groupId, teamMap) {
     ...
     */
    async _createAndRunFinal(seasonId, homeTeamId, awayTeamId, venueId, finalDelaySlots) {
        // Phase order
        const maxOrder = await this.prisma.phase.aggregate({
            where: { season_id: seasonId },
            _max: { order: true },
     ...
     */
    async _playFinalMatch(m, teamMap, venueId) {
        const homeName = teamMap.get(m.home_team_id) ?? `Team ${m.home_team_id}`;
        const awayName = teamMap.get(m.away_team_id) ?? `Team ${m.away_team_id}`;
        await this._createLineup(m.id, m.home_team_id, m.away_team_id);
        await this.matchLifecycleService.startMatch(m.id);
        ...
        if (isDraw) {
            ...
            if (awayPenalty === homePenalty)
                awayPenalty = homePenalty > 3 ? homePenalty - 1 : homePenalty + 1;
            await this.matchLifecycleService.finalizeMatch(m.id, {
    ...
    // ─── UTILS ────────────────────────────────────────────────────────────────
    _resolveWinnerName(finalMatch, groupAWinner, groupBWinner) {
        ...
        if (homeScore > awayScore)
            return groupAWinner.team;
        if (awayScore > homeScore)
            return groupBWinner.team;
        return 'Unknown';
    }
    /**
