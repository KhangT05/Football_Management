...
import { MatchResultStatus, Prisma, PrismaClient, SeasonFormat, SeasonStatus } from "../generated/prisma/client.js";
...
const isStageConfigArray = (v: unknown): v is StageConfig[] => customStagesSchema.safeParse(v).success;
...
const EDITABLE_SEASON_STATUSES: SeasonStatus[] = [
...
export class TournamentRuleService {
     ...
     */
    private validateCustomStages(stages: StageConfig[]): void {
        if (stages.length === 0)
        ...
        for (const stage of stages) {
            ...
            if (sourceOrder === null || sourceOrder === undefined) {
                if (stage.type === 'round_robin') continue; // round_robin sau vẫn được phép lấy toàn bộ approved pool mới (multi-stage RR song song, hiếm nhưng hợp lệ)
     ...
     */
    private async lockTournamentForActiveRuleWrite(
    ...
    async update(id: number, data: UpdateTournamentRuleDto, force = false): Promise<TournamentRuleDto> {
        ...
        if (lockedSeason) {
            throw createAppError(
                "RULE_LOCKED",
                `TournamentRule ${id}: season "${lockedSeason.name}" đang ở trạng thái ${lockedSeason.status} ` +
