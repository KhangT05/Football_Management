...
import { Prisma, PhaseFormat, PrismaClient, SeasonStatus, SeasonTeamStatus, PhaseStatus, PhaseType } from "../generated/prisma/client.js";
...
const GROUP_OPS_ALLOWED_SEASON_STATUS: SeasonStatus[] = [
...
export class GroupService {
    ...
    private async assertSeasonAcceptsGroupOps(tx: Prisma.TransactionClient, seasonId: number): Promise<void> {
        ...
        if (!GROUP_OPS_ALLOWED_SEASON_STATUS.includes(season.status))
            throw createAppError(
                "CONFLICT",
                `Season đang ở status '${season.status}' — chỉ được tạo/sửa group khi season ở ` +
     ...
     */
    private assertPhaseIsDraft(phase: { status: PhaseStatus }, action: string): void {
     ...
     */
    private async getEligibleApprovedTeams(
        tx: Prisma.TransactionClient,
        seasonId: number,
        phaseId: number,
    ): Promise<{ id: number; team_id: number; group_id: number | null; seed: number | null }[]> {
    ...
    private async countEligibleApprovedTeams(
        tx: Prisma.TransactionClient,
        seasonId: number,
        phaseId: number,
    ): Promise<number> {
    ...
    async previewGroupSplitBySeason(
        seasonId: number,
        desiredGroupCount: number,
    ): Promise<{ groupCount: number; distribution: number[]; warning?: string }> {
    ...
    previewGroupSplit(
        ...
        desiredGroupCount: number,
    ): { groupCount: number; distribution: number[]; warning?: string } {
    ...
    async createAndDrawGroups(seasonId: number, groupCount: number): Promise<DrawAssignment[]> {
        return this.prisma.$transaction(async (tx) => {
            const phase = await this.getOrCreateRoundRobinPhase(tx, seasonId);
            if (!phase.is_active) throw createAppError("CONFLICT", "Phase đã bị deactivate");
            this.assertPhaseIsDraft(phase, "tạo group mới");
            ...
            if (approvedTeams.length < groupCount * 2)
                throw createAppError(
                    "CONFLICT",
                    `Chỉ có ${approvedTeams.length} approved team, cần ít nhất ${groupCount * 2} cho ${groupCount} group`,
                );
    ...
    async autoFinalizeGroups(
        seasonId: number,
        opts: { minTeamsPerGroup?: number; maxTeamsPerGroup?: number } = {},
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (existingMatches > 0)
                throw createAppError(
                    "CONFLICT",
                    "Phase đã có match — không thể auto-finalize lại (xoá schedule trước nếu thực sự cần đổi số group)"
                );
            ...
            if (approvedTeams.length < minPerGroup)
                throw createAppError(
                    "CONFLICT",
                    `Chỉ có ${approvedTeams.length} approved team — không đủ ${minPerGroup} team tối thiểu để tổ chức 1 group`
                );
    ...
    async advanceToNextRoundRobin(
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            distributed.forEach((teamsInGroup, i) => {
                const group = newGroups[i]!;
                for (const st of teamsInGroup) assignments.push({ seasonTeamId: st.id, team_id: st.team_id, group_id: group.id });
            });
            await this.applyAssignments(tx, assignments);
    ...
    async findAllBySeason(seasonId: number) {
        ...
        if (!phase) return { phase: null, groups: [], eligibleTeamCount: null };
    ...
    async findAllByPhase(phaseId: number) {
        ...
        if (!phase || !phase.is_active) return { phase: null, groups: [], eligibleTeamCount: null };

        return this.buildGroupsPayload(phase);
    }

    private async buildGroupsPayload(phase: {
        id: number; name: string; format: PhaseFormat; status: PhaseStatus; order: number; teams_per_group: number | null; season_id: number;
    }) {
     ...
     */
    async confirmGroups(seasonId: number): Promise<void> {
        return this.prisma.$transaction(async (tx) => {
            await lockSeason(tx, seasonId);
            await this.assertSeasonAcceptsGroupOps(tx, seasonId);

            const phase = await this.findRoundRobinPhase(tx, seasonId);
            if (!phase) throw createAppError("CONFLICT", "Season chưa có group nào để xác nhận");
            if (phase.status === PhaseStatus.in_progress) return; // idempotent
            ...
            if (groups.length === 0) throw createAppError("CONFLICT", "Chưa có group nào để xác nhận");

            const empty = groups.filter((g) => g._count.season_teams < 2);
            if (empty.length > 0)
                throw createAppError(
                    "CONFLICT",
                    `${empty.length} group chưa đủ tối thiểu 2 đội (${empty.map((g) => g.name).join(", ")}) — ` +
     ...
     */
    async unconfirmGroups(seasonId: number): Promise<void> {
        return this.prisma.$transaction(async (tx) => {
            await lockSeason(tx, seasonId);

            const phase = await this.findRoundRobinPhase(tx, seasonId);
            if (!phase) throw createAppError("NOT_FOUND", "Season chưa có group");
            if (phase.status !== PhaseStatus.in_progress) return; // idempotent
    ...
    async createEmptyRoundRobinGroups(
        tx: Prisma.TransactionClient,
        seasonId: number,
        groupCount: number,
    ): Promise<{ phaseId: number; groups: { id: number; name: string }[] }> {
    ...
    async autoAssignApprovedTeamToGroup(
        tx: Prisma.TransactionClient,
        seasonId: number,
        seasonTeamId: number,
        ...
        for (const g of groups) {
            ...
            if (c < minCount) { minCount = c; target = g; }
        }
        if (!target)
            throw createAppError(
                "CONFLICT",
                `Tất cả group của phase ${phase.id} đã đầy — không thể auto-assign SeasonTeam ${seasonTeamId}`,
            );
