...
import {
    ...
    ASSUMED_MATCH_DURATION_MS,
    DEFAULT_VENUE_BUFFER_MINUTES,
} from '../libs/schedule.engine.js';
...
export class ScheduleService extends ScheduleEngine {
     ...
     */
    async getGroupStageRoundsSummary(
        seasonId: number,
        groupIds?: number[],
        ...
        const byRound = new Map<number, { total: number; unscheduled: number }>();
        for (const m of matches) {
            const r = parseInt(m.round ?? '0', 10);
            const e = byRound.get(r) ?? { total: 0, unscheduled: 0 };
            e.total++;
            if (m.scheduled_at === null) e.unscheduled++;
            byRound.set(r, e);
        }
        ...
        const { groupCount, groupIds } = await this.prisma.$transaction(
            async tx => {
                ...
                if (existingMatches > 0)
                    throw createAppError(
                        'CONFLICT',
                        `Season ${seasonId} đã có lịch thi đấu (${existingMatches} match) — dùng endpoint ` +
    ...
    async autoScheduleMatches(
        seasonId: number,
        options: ScheduleOptions & AutoScheduleFilterOptions & { allowPastDate?: boolean },
        ...
        const pool = this.buildSlotPool(
            options.venueIds, startDate, rangeEnd,
    ...
    async rescheduleMatch(matchId: number, input: RescheduleInput): Promise<void> {
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (venueConflict)
                throw createAppError(
                    'CONFLICT',
                    `Venue ${input.venueId} đã có trận (match ${venueConflict.id}) không đủ ` +
    ...
    async getScheduleDefaults(seasonId: number): Promise<{
        ...
        if (!season.default_daily_start_time) return null;
    ...
    async saveScheduleDefaults(
        ...
        if (validVenues.length !== uniqueVenueIds.length) {
            ...
            throw createAppError(
                'VALIDATION_ERROR',
                `venueIds không hợp lệ, đã inactive, hoặc đã bị xóa: [${invalid.join(', ')}]`,
            );
        }
