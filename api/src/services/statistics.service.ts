...
export type PlayerFinanceEntry = {
    ...
    bonus_earned: number;
    fine_owed: number;
    net_amount: number; // bonus_earned - fine_owed
};
...
const FINISHED_MATCH_STATUSES = ["finished", "forfeited"] as const;
...
export class StatisticsService {
    ...
    async getSeasonRevenue(seasonId?: number): Promise<SeasonRevenueStats> {
        ...
        const seasons: SeasonRevenue[] = rows.map((r) => {
    ...
    async getTournamentOverview(tournamentId: number): Promise<TournamentOverviewStats> {
        ...
        if (!tournament) {
            throw createAppError(
                "NOT_FOUND",
                `Tournament id=${tournamentId} not found`,
    ...
    async getTeamRegistrationStats(seasonId?: number): Promise<TeamRegistrationStats> {
        ...
        const bySeason = new Map<number, TeamRegistrationFunnel>();
        for (const r of rows) {
            ...
            switch (r.status) {
                case "pending": entry.pending_count = count; break;
                case "approved": entry.approved_count = count; break;
                case "active": entry.active_count = count; break;
                case "eliminated": entry.eliminated_count = count; break;
                case "withdrawn": entry.withdrawn_count = count; break;
            }
    ...
    async getTopScorers(seasonId: number, limit = 10): Promise<TopScorerStats> {
        if (limit <= 0 || limit > 100) {
            throw createAppError(
                "VALIDATION_ERROR",
                `getTopScorers called with invalid limit=${limit}`,
        ...
        const rows = await this.prisma.$queryRaw<
            {
                ...
                team_id: number; team_name: string; goal_count: bigint
            }[]
        > `
            SELECT
                pl.id AS player_id,
    ...
    async getTeamDisciplineStats(seasonId: number): Promise<TeamDisciplineStats> {
        const rows = await this.prisma.$queryRaw<
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // FIX: cùng bug player.name — đổi sang player.user.name.
    async getPlayerFinanceStats(seasonId: number): Promise<SeasonFinanceStats> {
        ...
        if (!season) {
            throw createAppError(
                "NOT_FOUND",
                `Season id=${seasonId} not found`,
                "Không tìm thấy season",
            );
        }
        if (!season.tournamentRule) {
            throw createAppError(
                "CONFLICT",
                `Season id=${seasonId} chưa gán tournament_rule_id`,
                "Season chưa có luật giải — không tính được thưởng/phạt",
            );
        }
        ...
        const players: PlayerFinanceEntry[] = rows
            .map((r) => {
    ...
    async getSystemOverviewStats(period = "30d"): Promise<SystemOverviewStats> {
        const days = PERIOD_DAYS[period];
        if (!days) {
            throw createAppError(
                "VALIDATION_ERROR",
                `getSystemOverviewStats called with invalid period=${period}`,
                "period không hợp lệ, dùng: 7d, 30d, 90d, 3m, 6m, 1y",
            );
        }
        ...
        const [
            ...
            this.prisma.$queryRaw<{ confirmed: string; refunded: string }[]>`
                SELECT
     ...
     */
    private async _fetchTeamMatchesWithRule(
        teamId: number,
        filter: { seasonId?: number; tournamentId?: number } = {},
    ): Promise<Array<{
        home_team_id: number;
        away_team_id: number;
     ...
     */
    private async _fetchTeamMatchesDetailed(
        teamId: number,
        filter: { seasonId?: number; tournamentId?: number; period?: string } = {},
    ): Promise<Array<{
        match_id: number;
        home_team_id: number;
        away_team_id: number;
        ...
        let playedAtFilter: Prisma.MatchWhereInput = {};
        if (filter.period) {
            const days = PERIOD_DAYS[filter.period];
            if (!days) {
                throw createAppError(
                    "VALIDATION_ERROR",
                    `_fetchTeamMatchesDetailed called with invalid period=${filter.period}`,
                    "period không hợp lệ, dùng: 7d, 30d, 90d, 3m, 6m, 1y",
                );
            }
    ...
    private _aggregateTeamMatchesExtended(
        teamId: number,
        matches: Awaited<ReturnType<StatisticsService["_fetchTeamMatchesDetailed"]>>,
    ): TeamAggregateStatsExtended {
        let wins = 0, draws = 0, losses = 0, goalsFor = 0, goalsAgainst = 0, points = 0;
        let cleanSheets = 0, forfeitWins = 0, forfeitLosses = 0;
        const home: TeamHomeAwaySplit = { matches_played: 0, wins: 0, draws: 0, losses: 0, goals_for: 0, goals_against: 0 };
        const away: TeamHomeAwaySplit = { matches_played: 0, wins: 0, draws: 0, losses: 0, goals_for: 0, goals_against: 0 };
        let biggestWin: TeamBiggestResult = null;
        let biggestLoss: TeamBiggestResult = null;
        ...
        for (const m of matches) {
            ...
            if (m.status === "forfeited") {
                if (gf > ga) forfeitWins++; else if (gf < ga) forfeitLosses++;
            }

            if (gf > ga) {
                wins++; side.wins++; points += m.points_per_win;
                const margin = gf - ga;
                ...
                losses++; side.losses++; points += m.points_per_loss;
                const margin = ga - gf;
                ...
                draws++; side.draws++; points += m.points_per_draw;
            }
        }

        // Streak: đi ngược từ trận gần nhất (cuối mảng, vì đã sort asc)
        let currentStreak: TeamStreakEntry = null;
        for (let i = matches.length - 1; i >= 0; i--) {
            ...
            const gf = isHome ? m.home_final_score : m.away_final_score;
            const ga = isHome ? m.away_final_score : m.home_final_score;
            const type: "W" | "D" | "L" = gf > ga ? "W" : gf < ga ? "L" : "D";
            if (!currentStreak) {
     ...
     */
    async getTeamOverviewStats(teamId: number): Promise<TeamOverviewStats> {
        ...
        const participations: TeamParticipation[] = seasonTeams.map((st) => ({
    ...
    async getTeamOverviewStatsV2(teamId: number, period?: string): Promise<TeamOverviewStats & { extended: TeamAggregateStatsExtended }> {
        const team = await this.prisma.team.findUnique({ where: { id: teamId }, select: { id: true, name: true } });
        if (!team) throw createAppError("NOT_FOUND", `Team id=${teamId} not found`, "Không tìm thấy đội");
    ...
    async getTeamTournamentStatsV2(teamId: number, tournamentId: number): Promise<TeamTournamentStats & { extended: TeamAggregateStatsExtended }> {
        ...
        if (!team) throw createAppError("NOT_FOUND", `Team id=${teamId} not found`, "Không tìm thấy đội");
        if (!tournament) throw createAppError("NOT_FOUND", `Tournament id=${tournamentId} not found`, "Không tìm thấy giải đấu");
    ...
    async getTeamSeasonStatsV2(teamId: number, seasonId: number): Promise<TeamSeasonStats & { extended: TeamAggregateStatsExtended }> {
        ...
        if (!team) throw createAppError("NOT_FOUND", `Team id=${teamId} not found`, "Không tìm thấy đội");
        if (!seasonTeam) throw createAppError("NOT_FOUND", `Team ${teamId} chưa tham gia season ${seasonId}`, "Đội chưa tham gia mùa giải này");
    ...
    async getTeamParticipationStats(teamId: number): Promise<TeamParticipationStats> {
        const team = await this.prisma.team.findUnique({ where: { id: teamId }, select: { id: true, name: true } });
        if (!team) throw createAppError("NOT_FOUND", `Team id=${teamId} not found`, "Không tìm thấy đội");
        ...
        for (const st of seasonTeams) status_breakdown[st.status]++;
    ...
    async getTeamsSeasonStatsBatch(seasonId: number): Promise<TeamSeasonStatsBatch> {
        const season = await this.prisma.season.findUnique({
            where: { id: seasonId },
            select: { id: true, tournamentRule: { select: { points_per_win: true, points_per_draw: true, points_per_loss: true } } },
        });
        if (!season) throw createAppError("NOT_FOUND", `Season id=${seasonId} not found`, "Không tìm thấy mùa giải");
        ...
        const rows = await this.prisma.$queryRaw<
            { team_id: number; team_name: string; wins: bigint; draws: bigint; losses: bigint; matches_played: bigint; goals_for: string; goals_against: string }[]
        > `
        SELECT
            t.id AS team_id,
            t.name AS team_name,
    ...
    async getPlayerParticipationStats(playerId: number): Promise<PlayerParticipationStats> {
        const player = await this.prisma.player.findUnique({
            where: { id: playerId },
            select: { id: true, user: { select: { name: true } } },
        });
    ...
    async getPlayerTeamsInPeriod(playerId: number, from: Date, to: Date): Promise<PlayerTeamsInPeriodStats> {
        ...
        const entries: PlayerTeamsInPeriodEntry[] = rows.map((r) => ({
            team_id: r.team.id,
            team_name: r.team.name,
            season_id: r.season.id,
            season_name: r.season.name,
     ...
     */
    async getTeamTournamentStats(teamId: number, tournamentId: number): Promise<TeamTournamentStats> {
        const [team, tournament, seasonTeams] = await Promise.all([
            this.prisma.team.findUnique({ where: { id: teamId }, select: { id: true, name: true } }),
            this.prisma.tournament.findUnique({ where: { id: tournamentId }, select: { id: true, name: true } }),
            this.prisma.seasonTeam.findMany({
     ...
     */
    async getTeamSeasonStats(teamId: number, seasonId: number): Promise<TeamSeasonStats> {
        const [team, seasonTeam] = await Promise.all([
            this.prisma.team.findUnique({ where: { id: teamId }, select: { id: true, name: true } }),
            this.prisma.seasonTeam.findUnique({
                where: { season_id_team_id: { season_id: seasonId, team_id: teamId } },
        ...
        if (!seasonTeam) {
            throw createAppError(
                "NOT_FOUND",
                `Team ${teamId} chưa tham gia season ${seasonId}`,
                "Đội chưa tham gia mùa giải này",
            );
        }
     ...
     */
    async getTeamMatchStats(teamId: number, matchId: number): Promise<TeamMatchStats> {
        ...
        if (match.home_team_id !== teamId && match.away_team_id !== teamId) {
            throw createAppError(
                "VALIDATION_ERROR",
                `Team ${teamId} không thi đấu trong match ${matchId}`,
                "Đội không thi đấu trong trận này",
            );
        }
        ...
        let result: TeamMatchStats["result"] = "pending";
    ...
    async getTeamMatchTimeSeries(
        ...
        period?: string,
    ): Promise<TeamMatchTimeSeriesStats> {
        if (!["day", "month", "year"].includes(granularity)) {
            throw createAppError(
                "VALIDATION_ERROR",
                `getTeamMatchTimeSeries called with invalid granularity=${granularity}`,
                "granularity không hợp lệ, dùng: day, month, year",
            );
        }

        const tzOffset = `+${String(BUSINESS_TZ_OFFSET_HOURS).padStart(2, "0")}:00`;
        const dateFormatMap: Record<TimeGranularity, string> = {
            day: "%Y-%m-%d",
            month: "%Y-%m",
            year: "%Y",
        };
        const bucketExpr = Prisma.raw(
            `DATE_FORMAT(CONVERT_TZ(m.played_at, '+00:00', '${tzOffset}'), '${dateFormatMap[granularity]}')`,
        );
        ...
        if (period) {
            ...
            if (!days) {
                throw createAppError(
                    "VALIDATION_ERROR",
                    `getTeamMatchTimeSeries called with invalid period=${period}`,
                    "period không hợp lệ, dùng: 7d, 30d, 90d, 3m, 6m, 1y",
                );
            }
        ...
        const rows = await this.prisma.$queryRaw<
            { bucket: string; wins: bigint; draws: bigint; losses: bigint; matches_played: bigint }[]
        >(Prisma.sql`
            SELECT
    ...
    async getPlayerOverviewStats(playerId: number): Promise<PlayerOverviewStats> {
        const player = await this.prisma.player.findUnique({
            where: { id: playerId },
            select: { id: true, user: { select: { name: true } } },
        });
     ...
     */
    async getPlayerTournamentStats(playerId: number, tournamentId: number): Promise<PlayerTournamentStats> {
    ...
    async getPlayerSeasonStats(playerId: number, seasonId: number): Promise<PlayerSeasonStats> {
        ...
        if (rows.length === 0) {
            throw createAppError(
                "NOT_FOUND",
                `Player ${playerId} chưa có dữ liệu ở season ${seasonId}`,
                "Cầu thủ chưa thi đấu ở mùa giải này",
            );
        }

        const teams: PlayerSeasonTeamBreakdown[] = rows.map((r) => ({
            team_id: r.team.id,
            team_name: r.team.name,
            matches_played: r.matches_played,
            goals: r.goals_scored,
     ...
     */
    async getPlayerMatchStats(playerId: number, matchId: number): Promise<PlayerMatchStats> {
        ...
        if (!lineup) {
            throw createAppError(
                "NOT_FOUND",
                `Player ${playerId} không có trong đội hình match ${matchId}`,
                "Cầu thủ không thi đấu trận này",
            );
        }
        ...
        return {
            ...
            played_at: match.played_at ? match.played_at.toISOString() : null,
            lineup_type: lineup.lineup_type as "starter" | "substitute",
            minutes_played: minutesPlayed,
            is_captain: lineup.is_captain,
            goals: match.events.filter((e) => e.type === "goal" || e.type === "penalty_scored").length,
            yellow_cards: match.events.filter((e) => e.type === "yellow_card" || e.type === "second_yellow").length,
            red_cards: match.events.filter((e) => e.type === "red_card").length,
            events: match.events.map((e) => ({ minute: e.minute, type: e.type as string })),
     ...
     */
    async getTeamsFinanceStatsBatch(seasonId: number): Promise<{ season_id: number; teams: TeamFinanceEntry[] }> {
        const season = await this.prisma.season.findUnique({
            where: { id: seasonId },
            select: {
        ...
        const [paymentRows, statRows, teams] = await Promise.all([
            this.prisma.$queryRaw<{ team_id: number; paid: string; refunded: string }[]>`
            SELECT
     ...
     */
    async getPlayersPerformanceStatsBatch(seasonId: number): Promise<{ season_id: number; players: PlayerPerformanceBatchEntry[] }> {
        const [statRows, lineupRows] = await Promise.all([
            ...
            this.prisma.$queryRaw<
                { player_id: number; starter_count: bigint; sub_count: bigint; captain_count: bigint }[]
            > `
            SELECT
 ...
 */
    async getTeamFinanceStats(teamId: number, seasonId: number): Promise<TeamFinanceEntry> {
        const batch = await this.getTeamsFinanceStatsBatch(seasonId);
        const entry = batch.teams.find((t) => t.team_id === teamId);
        if (!entry) {
            throw createAppError(
                "NOT_FOUND",
                `Team ${teamId} không có dữ liệu finance ở season ${seasonId}`,
                "Đội chưa tham gia mùa giải này hoặc chưa có dữ liệu tài chính",
            );
        }
        return entry;
    }
