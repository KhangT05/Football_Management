...
// ============================================================
// Shape của 1 phần tử trong TournamentRule.custom_stages (Json column) —
// PHẢI khớp 1-1 với convertCustomStagesForApi() ở TournamentWizardModal.jsx
// (FE convert _cid -> order số nguyên đúng 1 lần lúc submit). Đây là "source
// of truth" phía BE cho shape đó — nếu FE đổi field nào, sửa type này theo.
//
// TODO-CONFIRM: giả định custom_stages lưu trong TournamentRule dưới dạng
// cột Json (Prisma `Json` type), đọc ra là `unknown` cần cast/validate. Nếu
// schema thật có bảng con riêng (vd TournamentRuleStage[]) thay vì Json blob,
// phần load ở dưới (loadCustomStages) cần đổi query — logic advance() còn
// lại không đổi vì chỉ phụ thuộc shape này, không phụ thuộc cách lưu trữ.
//
// GAP QUAN TRỌNG (xem giải thích cuối file, phần "Điểm số per-stage"):
// points_per_win/draw/loss khai báo ở đây HIỆN KHÔNG ĐƯỢC standing.service.ts
// đọc — recomputeGroupStandings() chỉ đọc season.tournamentRule.points_per_*
// (1 bộ điểm chung toàn season). 3 field này trong CustomStageRoundRobin chỉ
// mang tính khai báo/preview ở FE cho tới khi vấn đề đó được giải quyết.
// ============================================================
...
export type AdvancePipelineResult = {
    ...
    startedStages: {
        ...
        phaseId: number | null; // null nếu stage cần thao tác thủ công (vd seed_mode='manual')
...
export class CustomPipelineService {
    ...
    async advanceFromPhase(
        ...
        if (season.tournamentRule?.format !== SeasonFormat.custom)
            throw createAppError(
                'VALIDATION_ERROR',
                `Season ${seasonId} không dùng format 'custom' — pipeline tự động chỉ áp dụng cho format này. ` +
        ...
        if (!fromPhase) throw createAppError('NOT_FOUND', `Phase ${fromPhaseId} không tồn tại`);
        ...
        if (fromPhase.custom_stage_order === null)
            throw createAppError(
                'CONFLICT',
                `Phase ${fromPhaseId} không có custom_stage_order — phase này không được tạo bởi ` +
        ...
        if (fromPhase.status !== PhaseStatus.locked)
            throw createAppError(
                'CONFLICT',
                `Phase ${fromPhaseId} chưa locked (status hiện tại: ${fromPhase.status}) — chỉ advance được ` +
        ...
        const startedStages: AdvancePipelineResult['startedStages'] = [];
    ...
    private async startRoundRobinStage(
        seasonId: number,
        fromPhase: { id: number; format: PhaseFormat },
        stage: CustomStageRoundRobin,
    ): Promise<AdvancePipelineResult['startedStages'][number]> {
        if (fromPhase.format !== PhaseFormat.round_robin)
            throw new Error(
                `Nguồn của stage round_robin "${stage.name}" phải là 1 stage round_robin đã locked ` +
        ...
        const { newPhaseId } = await this.groupService.advanceToNextRoundRobin(
            fromPhase.id,
            stage.group_count,
            { from, to },
        );
    ...
    private async startKnockoutStage(
        seasonId: number,
        fromPhase: { id: number; format: PhaseFormat },
        stage: CustomStageKnockout,
        scheduleOptions: OptionalScheduleOptions,
    ): Promise<AdvancePipelineResult['startedStages'][number]> {
        ...
        if (fromPhase.format !== PhaseFormat.round_robin)
            throw new Error(
                `Stage knockout "${stage.name}" seed từ standings nhưng nguồn không phải round_robin — ` +
    ...
    private async startClassificationStage(
        seasonId: number,
        fromPhase: { id: number; format: PhaseFormat },
        stage: CustomStageClassification,
        scheduleOptions: OptionalScheduleOptions,
    ): Promise<AdvancePipelineResult['startedStages'][number]> {
        ...
        throw new Error(
            `Stage classification "${stage.name}" (source_kind=${stage.source_kind}) chưa có service method ` +
    ...
    private loadCustomStages(raw: unknown): CustomStage[] {
