...
import { ScheduleEngine, ScheduleCandidateMatch, DEFAULT_VENUE_BUFFER_MINUTES } from '../libs/schedule.engine.js';
...
const KNOCKOUT_STAGE_PREREQUISITE: Partial<Record<PhaseType, PhaseType>> = {
...
export class KnockoutService extends ScheduleEngine {
     ...
     * đọc position.
     */
    private async _ensureGroupsReadyAndRecompute(
        tx: Prisma.TransactionClient,
        groupIds: number[],
    ): Promise<void> {
     ...
     */
    async generateKnockoutFromStandings(options: AutoSeedKnockoutOptions): Promise<KnockoutGenerateResult> {
        ...
        if ((options.mode === 'straight' || options.mode === 'cross') && options.groupIds.length % 2 !== 0)
            throw createAppError(
                'VALIDATION_ERROR',
                `Mode '${options.mode}' cần số group chẵn (ghép cặp group liên tiếp) — hiện có ${options.groupIds.length} group`,
            );
        ...
        if (!Number.isInteger(Math.log2(totalTeams)))
            throw createAppError(
                'VALIDATION_ERROR',
                `Tổng số đội vào knockout (${options.groupIds.length} group × top${options.topN} = ${totalTeams}) ` +
        ...
        if (!phaseType)
            throw createAppError(
                'VALIDATION_ERROR',
                `${totalTeams} đội không map được sang PhaseType chuẩn — truyền phaseTypeOverride`,
            );
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            const byGroup = new Map<number, { position: number; teamId: number }[]>();
            for (const gid of options.groupIds) byGroup.set(gid, []);
            for (const r of rows) byGroup.get(r.group_id)!.push({ position: r.position, teamId: r.team_id });
            ...
            for (const gid of options.groupIds) {
                ...
                if (list.length < options.topN)
                    throw createAppError(
                        'CONFLICT',
                        `Group ${gid} chỉ có ${list.length} standing, cần top${options.topN} — ` +
            ...
            const groupOfTeam = new Map<number, number>();
            for (const [gid, list] of byGroup) for (const t of list) groupOfTeam.set(t.teamId, gid);
    ...
    private _buildStraightPairs(
        groupIds: number[],
        byGroup: Map<number, { position: number; teamId: number }[]>,
        topN: number,
    ...
    private _buildRandomPairs(
        ...
        const shuffledByGroup = new Map(
            groupIds.map(gid => [gid, shuffle(byGroup.get(gid)!.map(t => t.teamId))]),
        );
    ...
    private async _finalizeWithSchedule(
        result: { phaseId: number; phaseType: PhaseType; totalSlots: number; round1MatchIds: number[]; byeSlots: number; warnings: string[] },
        seasonId: number,
     ...
     */
    private async _buildBracketInPhase(
        tx: Prisma.TransactionClient,
        seasonId: number,
        phaseType: PhaseType,
        legsOption: 1 | 2,
    ...
    private async scheduleMatchBatch(
        ...
        const pool = this.buildSlotPool(
            options.venueIds, startDate, rangeEnd, options.dailyStartTime,
            options.dailyEndTime, bufferMinutes, takenWindows,
 ...
 */
    private async _assertPreviousStageComplete(
        tx: Prisma.TransactionClient,
        seasonId: number,
        phaseType: PhaseType,
    ): Promise<void> {
        ...
        if (unfinishedCount > 0)
            throw createAppError(
                'CONFLICT',
                `Còn ${unfinishedCount} trận '${priorType}' chưa kết thúc — hoàn thành trước khi tạo '${phaseType}'`,
            );
    }
     ...
     */
    async getStageReadiness(seasonId: number, targetPhaseType: PhaseType): Promise<{
        ready: boolean;
        ...
        if (!priorType) return { ready: true, priorPhaseType: null, priorPhaseExists: true, unfinishedCount: 0 };
