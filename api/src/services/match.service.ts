...
import {
    ...
    findAdvancedChildMatchId, isKnockoutBracketSeeded,
        ...
        for (const matchId of autoIds) {
            ...
            try {
                ...
                if (nextRetryCount >= MAX_GRACE_PERIOD_RETRY) {
                    console.error(
                        `[GracePeriod] match ${matchId} fail ${nextRetryCount} lần liên tiếp — ` +
    ...
    private async _recalculateResultTx(
        ...
        if (!isKnockout) {
            ...
            if (seeded)
                throw createAppError(
                    'FORBIDDEN',
                    `Match ${matchId}: knockout bracket của season ${seasonId} đã seed — không thể sửa ` +
        ...
        if (isKnockout && winnerTeamId !== match.matchResult.winner_team_id) {
            ...
            if (childMatchId !== null)
                throw createAppError(
                    'CONFLICT',
                    `Match ${matchId}: winner đổi từ ${match.matchResult.winner_team_id} sang ` +
