...
// 11 "flavor" khác nhau — bao phủ gần hết các nhánh trạng thái Match có trong schema.
type Flavor =
    | "finished_official"
    | "finished_protested"
...
const COUNTS_TOWARD_STANDINGS: ReadonlySet<Flavor> = new Set([
    "finished_official",
    "finished_protested",
    "finished_under_review",
    "forfeited",
...
export interface MessyGroupStageResult {
    createdMatches: CreatedMatchInfo[]; // chỉ những trận đã có tỉ số CHÍNH THỨC -> để chạy tiếp matchDetailSeeder
    scheduledGroupLetters: GroupLetter[]; // các bảng có status SCHEDULED (có match)
    draftGroupLetters: GroupLetter[]; // các bảng DRAFT (chưa sinh lịch — "Trống")
    scheduleFailedGroupLetters: GroupLetter[]; // các bảng SCHEDULE_FAILED (sinh lịch lỗi — "Trống")
}
 ...
 */
async function findExistingFlavorMatch(
    db: DbClient,
    phaseId: number,
    groupId: number,
    homeTeamId: number,
    awayTeamId: number
): Promise<{ matchId: number; homeScore: number | null; awayScore: number | null; hasOfficialResult: boolean } | null> {
    ...
    const officialStatuses: ReadonlySet<MatchResultStatus> = new Set([
    ...
    const hasOfficialResult =
        matchResult?.home_final_score != null &&
...
async function createFlavorMatch(
    db: DbClient,
    flavor: Flavor,
    phaseId: number,
    groupId: number,
    ...
    forfeitScore: number,
    round: number
): Promise<{ matchId: number; homeScore: number | null; awayScore: number | null }> {
    const now = Date.now();
...
export async function seedMessyGroupStageMatches(
    db: DbClient,
    ...
    forfeitScore: number
): Promise<MessyGroupStageResult> {
    const letters = Object.keys(groups) as GroupLetter[];
    const createdMatches: CreatedMatchInfo[] = [];
    const scheduledGroupLetters: GroupLetter[] = [];
    const draftGroupLetters: GroupLetter[] = [];
    const scheduleFailedGroupLetters: GroupLetter[] = [];
    ...
    for (let gi = 0; gi < letters.length; gi++) {
        ...
        if (groupId === undefined) throw new Error(`messySeasonSeeder: thiếu group_id cho bảng "${letter}"`);
        ...
        if (!teamNames) throw new Error(`messySeasonSeeder: groups thiếu bảng "${letter}"`);
        const teamIds = teamNames.map((n) => {
            const id = teamIdByName[n];
            if (id === undefined) throw new Error(`Không tìm thấy team_id cho đội "${n}" (bảng ${letter})`);
            return id;
        });
        ...
        const groupCreatedMatches: CreatedMatchInfo[] = [];
        ...
        for (let pairIdx = 0; pairIdx < ROUND_ROBIN_PAIRS.length; pairIdx++) {
            ...
            if (existing) {
                ...
                const created = await createFlavorMatch(
                    db,
                    flavor,
                    groupStagePhaseId,
                    groupId,
                    homeTeamId,
                    awayTeamId,
                    pickOrThrow(venueIds, "messySeasonSeeder venueIds"),
                    globalDayOffset,
                    forfeitScore,
                    pairIdx + 1
                );
...
export async function seedMatchDetailsForMessyMatches(
    db: DbClient,
