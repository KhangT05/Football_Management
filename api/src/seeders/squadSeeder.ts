...
import { randInt, pickOrThrow, buildSquadPositions, generatePlayerEmail, generatePlayerName, slugifyTeamName } from "./helperSeeder.js";
 ...
 */
async function assertNoCrossTeamConflictInSeason(db: PrismaClient, seasonTeamId: number, playerId: number): Promise<void> {
  ...
  if (conflict) {
    throw new Error(
      `[SquadSeeder] player #${playerId} đã thuộc season_team #${conflict.season_team_id} trong season #${st.season_id} — ` +
...
export async function seedSquads(
  db: PrismaClient,
  seasonTeamIdByName: Record<string, number>,
  ...
  for (const [teamName, seasonTeamId] of teamEntries) {
    ...
    console.log(
      `[SquadSeeder] ${teamName} (season_team #${seasonTeamId}): đủ ${Math.min(alreadyCount + slotsToFill, targetSquadSize)}/${SQUAD_SIZE} cầu thủ (target=${targetSquadSize})`
    );
  }
}
 ...
 */
function buildProportionalPositions(size: number): PlayerPosition[] {
  ...
  while (result.length < size) result.push(PlayerPosition.defender);
 ...
 */
export async function seedBoundarySquad(
  db: PrismaClient,
  seasonTeamId: number,
  teamNameForEmail: string,
  size: number // truyền đúng min_players_per_team hoặc max_players_per_team từ TournamentRule
): Promise<void> {
  const passwordHash = await bcrypt.hash(DEFAULT_PASSWORD, BCRYPT_ROUNDS);
  ...
  for (let jersey = already + 1; jersey <= size; jersey++) {
    ...
    const position = positions[jersey - already - 1] as PlayerPosition;
    const player = await db.player.upsert({
      where: { user_id: user.id },
      update: {},
 ...
 */
export async function seedGoalkeeperlessSquad(db: PrismaClient, seasonTeamId: number, teamNameForEmail: string, size: number = 11): Promise<void> {
  const passwordHash = await bcrypt.hash(DEFAULT_PASSWORD, BCRYPT_ROUNDS);
  const slug = slugifyTeamName(teamNameForEmail);
  ...
  for (let jersey = already + 1; jersey <= size; jersey++) {
    ...
    const position = outfield[(jersey - 1) % outfield.length] as PlayerPosition;
    const player = await db.player.upsert({
      where: { user_id: user.id },
      update: {},
      create: { user_id: user.id, date_of_birth: new Date(2001, 0, 1), position, nationality: teamNameForEmail },
 ...
 */
export async function seedMidSeasonTransfer(
  db: PrismaClient,
  fromSeasonTeamId: number,
  toSeasonTeamId: number,
  playerId: number,
  newJerseyNumber: number,
  reason: LeaveReason = LeaveReason.transferred
): Promise<void> {
