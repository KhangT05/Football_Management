...
// Kiểu tối giản dùng chung cho match sau TX#1 (từ groupMatchSeeder/knockoutSeeder)
// hoặc nạp lại từ DB khi TX#1 bị skip vì đã seed từ trước.
...
export async function seedDatabase() {
    ...
    if (understaffed.length > 0) {
        throw new Error(
            `[Index] ${understaffed.length} đội chưa đủ 11 cầu thủ sau seedSquads: ` +
    ...
    if (groupKnockoutDone) {
        ...
        if (!finalMatch?.matchResult?.winner_team_id) {
            throw new Error(
                `[Index] Season #${seasonId}: final match có result nhưng thiếu winner_team_id — ` +
        ...
        const tx1Result = await prisma.$transaction(async (tx) => {
            ...
            const { topTwoByGroup, createdMatches: groupMatches } = await seedGroupMatchesAndStandings(
                ...
                venueIds,
                groups,
                rulePoints // NEW — thay hardcode +3/+1/+0 trong groupMatchSeeder
            );
            ...
            return {
                allMatches: [...groupMatches, ...knockoutMatches] as SeededMatchInfo[],
                championTeamId,
            };
    ...
    if (matchesNeedingDetail.length === 0) {
        ...
        console.log(
            `[Index] ${matchesNeedingDetail.length}/${allMatches.length} trận CHƯA có lineup ` +
        ...
        for (let i = 0; i < matchesNeedingDetail.length; i += MATCH_DETAIL_BATCH_SIZE) {
            ...
            console.log(
                `[Index] Batch ${Math.floor(i / MATCH_DETAIL_BATCH_SIZE) + 1}/` +
    ...
    if (matchesMissingLineup > 0) {
        console.warn(
            `[Index] ⚠️ ${matchesMissingLineup} trận KHÔNG có lineup dù đã có tỉ số. ` +
