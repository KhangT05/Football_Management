 ...
 */
export async function seedPartialRegistrations(
    db: DbClient,
    seasonId: number,
    availableTeamIds: number[],
    plan: RegistrationPlanEntry[]
): Promise<PartialRegistrationResult> {
    const seasonTeamIdByTeamId: Record<number, number> = {};
    const registeredTeamIds: number[] = [];
    ...
    for (const entry of plan) {
        for (let i = 0; i < entry.count; i++) {
            ...
            if (teamId === undefined) {
                console.warn(
                    `[RegistrationSeeder] hết đội khả dụng (cần thêm ${entry.count - i} đội status=${entry.status}) — dừng sớm.`
                );
                break;
            }
    ...
    console.log(
        `[RegistrationSeeder] Season #${seasonId}: đăng ký ${registeredTeamIds.length}/${availableTeamIds.length} đội khả dụng ` +
