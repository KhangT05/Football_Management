 ...
 */
async function getSquadOrdered(db: DbClient, seasonTeamId: number): Promise<TeamPlayerLite[]> {
...
async function seedCardEvents(db: DbClient, matchId: number, teamId: number, starters: TeamPlayerLite[], maxMinute: number = 90) {
 ...
 */
export async function seedForfeitMatchDetails(
    db: DbClient,
    params: {
        matchId: number;
        ...
        forfeitScore: number; // lấy từ TournamentRule.forfeit_score của season tương ứng
 ...
 * trong schema).
 */
export async function seedAbandonedMatchDetails(
    db: DbClient,
    params: {
        matchId: number;
        homeTeamId: number;
    ...
    const {
        matchId, homeTeamId, awayTeamId, homeSeasonTeamId, awaySeasonTeamId,
        abandonedMinute, abandonedReason, homeScoreAtAbandonment, awayScoreAtAbandonment,
    } = params;
 ...
 * thức.
 */
export async function seedDisputedResultDetails(
    db: DbClient,
    params: {
        matchId: number;
