...
import { PrismaClient, MatchEventType } from "../generated/prisma/client.js";
 ...
 */
export async function seedPlayerStatistics(db: PrismaClient, seasonId: number): Promise<void> {
    const matches = await db.match.findMany({
        where: { phase: { season_id: seasonId } },
        select: { id: true },
    });
    ...
    const key = (playerId: number, teamId: number) => `${playerId}:${teamId}`;
    ...
    for (const l of lineups) {
        ...
        if (l.lineup_type === "starter") a.minutes += 90;
    }
    ...
    for (const e of events) {
        ...
        const a = acc.get(k);
        if (!a) continue;
        if (e.type === MatchEventType.goal || e.type === MatchEventType.penalty_scored) a.goals++;
        if (e.type === MatchEventType.yellow_card) a.yellow++;
        if (e.type === MatchEventType.red_card || e.type === MatchEventType.second_yellow) a.red++;
    }
