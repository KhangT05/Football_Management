...
async function seedRoundOf16(
    ...
    venueIds: number[]
): Promise<{ phaseId: number; results: RoundResult[] }> {
...
export async function seedKnockoutBracket(
    ...
    const qf = await advanceKnockoutRound(
        db,
        seasonId,
        r16.results,
        { name: "Tứ kết", type: PhaseType.quarter_final, order: 3 },
        venueIds
    );

    const sf = await advanceKnockoutRound(
        db,
        seasonId,
        qf.results,
        { name: "Bán kết", type: PhaseType.semi_final, order: 4 },
        venueIds
    );

    const thirdPlace = await seedThirdPlaceMatch(db, seasonId, sf.results, 5, venueIds);

    const final = await advanceKnockoutRound(
        db,
        seasonId,
        sf.results,
        { name: "Chung kết", type: PhaseType.final, order: 6 },
        venueIds
    );
