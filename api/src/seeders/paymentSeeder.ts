...
const DEFAULT_WEIGHTS: PaymentStatusWeights = {
    confirmed: 0.85,
    pending: 0.1,
    rejected: 0,
    refund_pending: 0,
...
function rollStatus(weights: PaymentStatusWeights): PaymentStatus {
    const entries: [PaymentStatus, number][] = [
    ...
    if (total <= 0) return PaymentStatus.pending;
    let roll = Math.random() * total;
    for (const [status, w] of entries) {
        if (roll < w) return status;
        roll -= w;
    }
...
export async function seedPayments(
    ...
    weights: PaymentStatusWeights = DEFAULT_WEIGHTS,
    ...
    for (const seasonTeamId of seasonTeamIds) {
        ...
        await db.payment.create({
            data: {
                ...
                refunded_at:
                    status === PaymentStatus.refunded ? new Date() : status === PaymentStatus.refund_pending ? null : null,
...
export const FULL_SPREAD_WEIGHTS: PaymentStatusWeights = {
    confirmed: 0.4,
    pending: 0.25,
    rejected: 0.15,
    refund_pending: 0.1,
