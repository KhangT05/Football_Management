...
import { PrismaClient, Prisma, JerseyType, SeasonStatus, PitchType } from "../generated/prisma/client.js";
...
export interface SeasonConfig {
  ...
  pitchType?: PitchType;
  description?: string;
  // Nếu true (mặc định, giữ hành vi cũ), tự động đăng ký + tạo jersey cho
  // TOÀN BỘ team trong teamIdByName ngay khi tạo Season — phù hợp mùa giải
  // "finished"/"ongoing" nơi mọi đội đã chắc chắn tham dự.
  // Nếu false, Season được tạo TRỐNG (không SeasonTeam nào) — dùng cho mùa
  // "upcoming"/"registration_open"/"cancelled" nơi việc đăng ký đội cần mô
  // phỏng chi tiết hơn (pending/withdrawn/rejected-payment...) — xem
  // registrationSeeder.ts.
 ...
 */
export async function seedSeasonConfigurable(
  db: PrismaClient,
  config: SeasonConfig,
  ...
  console.log(
    `[SeasonSeeder] Season #${season.id} (${config.name}, status=${config.status}) với ${Object.keys(seasonTeamIdByTeamId).length} SeasonTeam`
  );
