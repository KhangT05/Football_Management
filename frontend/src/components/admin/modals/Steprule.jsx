...
export default function StepRule() {
    ...
    const selectedTemplate = ruleTemplates.find((r) => String(r.id) === String(ruleSourceId)) ?? null;
    ...
    useEffect(() => {
        ...
        if (meta.value === 'custom') {
            ...
            if (!current || current.length === 0) setValue('rule.custom_stages', seedInitialCustomStage());
            return;
        }
