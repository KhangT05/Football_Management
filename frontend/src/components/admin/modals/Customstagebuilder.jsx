...
// import { createDefaultStage, type CustomStage } from '../../schemas/wizard.schema';
...
//     const stages = useWatch({ control, name: 'rule.custom_stages' }) as CustomStage[];

//     const addStage = (type: CustomStage['type']) => {
...
//     const updateSource = (index: number, sourceCid: string | null) => {
//         const s = stages[index];
//         const patch: Partial<CustomStage> = { source_stage_cid: sourceCid };
//         if (s.type === 'round_robin') (patch as any).source_rank_range = sourceCid ? [1, s.teams_advance_per_group || 1] : null;
//         if (s.type === 'classification') (patch as any).source_kind = 'standing';
//         update(index, { ...s, ...patch } as CustomStage);
//     };
...
//                                 <input className={INPUT} {...register(`rule.custom_stages.${index}.name` as const)} placeholder="VD: Vòng bảng, Bán kết, Tranh hạng 3" />
//                             </FormField>
...
//                                         <select className={INPUT} {...register(`rule.custom_stages.${index}.seed_mode` as const)}>
//                                             {SEED_MODE_META.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
//                                         </select>
//                                     </FormField>
//                                     <FormField label="Thể thức sân đấu" required>
//                                         <select className={INPUT} {...register(`rule.custom_stages.${index}.leg_type` as const)}>
...
//                                         <select className={INPUT} {...register(`rule.custom_stages.${index}.source_kind` as const)} disabled={sourceStage?.type !== 'knockout'}>
