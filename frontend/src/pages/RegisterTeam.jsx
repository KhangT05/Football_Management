...
import { UploadCloud, ShieldCheck, CheckCircle2, Trophy, Loader2, ArrowRight, X, Users, Plus, Trash2, Info, FileSpreadsheet, Upload, FileDown } from 'lucide-react';
...
export default function RegisterTeam() {
  ...
  // ── Step control: 'info' (tạo team) → 'roster' (nhập đội hình) ──
  ...
  // FIX #4: nút "Tải file mẫu" trước đây là <a href="#"> nên bấm vào bị nhảy
  // lên đầu trang (anchor "#"). Đổi thành hàm gọi API tải file thật.
  ...
  const handleCreateTeam = async (e) => {
    ...
    try {
      ...
      setStep('roster'); // FIX #3: ẩn form tạo team, chuyển sang màn hình đội hình
    } catch (error) {
  ...
  const handleAddPlayers = async () => {
    ...
    try {
      if (playerInputMode === 'excel' && excelFile) {
        ...
        // FIX: bỏ luồng tự chế search-user/tạo-user-password-cứng cũ (không an
        // toàn, không gửi invite, dễ race-condition). Dùng lại đúng 1 API
        // createForTeam (giống MyTeam.jsx) — service tự lo find-or-create user
        // + issue invite token + gửi mail, trong 1 transaction.
  ...
  return (
    <div className="min-h-screen relative pt-24 pb-20 overflow-x-hidden bg-navy-dark">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* ─────────────────────────────────────────────────────
            BƯỚC 1: Form tạo Team — ẩn hoàn toàn khi step === 'roster'
        ───────────────────────────────────────────────────── */}
        {step === 'info' && (
        ...
        {/* ─────────────────────────────────────────────────────
            BƯỚC 2: Chỉ hiện sau khi team đã được tạo — chỉ import cầu thủ,
            KHÔNG tạo lại team.
        ───────────────────────────────────────────────────── */}
        {step === 'roster' && (
            ...
            <div className="bg-navy/60 backdrop-blur-2xl border border-navy-light rounded-[2.5rem] p-6 md:p-12 shadow-2xl shadow-black/40">
              ...
              {playerInputMode === 'excel' ? (
                <div className="animate-fade-in">
                  <div className="bg-navy border border-navy-light p-4 rounded-xl flex items-start gap-3 mb-8">
                    ...
                    <div className="text-sm text-red-400 leading-relaxed">
                      ...
                      <br />
                      {/* FIX #4: dùng button + hàm tải file thật, không dùng <a href="#"> nữa */}
                      <button
                        type="button"
                ...
                <div className="animate-fade-in">
                  ...
                  {/* FIX #1: bỏ đoạn ghi chú lỗi thời "cầu thủ cần có Tài khoản đã đăng ký"
                      — hệ thống hiện đã tự tạo tài khoản theo email nếu chưa tồn tại. */}
                  <div className="bg-navy border border-navy-light p-4 rounded-xl flex items-start gap-3 mb-8">
